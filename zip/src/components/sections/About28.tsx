...
export const About28 = ({
  ...
  return (
    <section
      className={cn(
        "mt-10 overflow-x-hidden bg-white py-20 md:mt-14 lg:mt-20",
        className,
      )}
    >
      <div className="theme-section-wrapper">
        <div className="mx-auto flex max-w-5xl flex-col-reverse gap-8 md:gap-14 lg:flex-row lg:items-end">
          {/* Images Left - Text Right */}
          <div className="flex w-full flex-col gap-8 lg:w-1/2 lg:gap-16 xl:gap-20">
    ...
    <div className="flex-1 space-y-4 text-lg md:space-y-6">
      {title && (
        <div className="space-y-6">
          ...
          <h2 className="font-headline text-4xl uppercase text-foreground">{title}</h2>
        </div>
      )}
      <div className="max-w-xl space-y-6 text-muted-foreground">
        {paragraphs.map((paragraph, index) => (
          <p key={index} className="font-body text-sm text-zinc-500">{paragraph}</p>
        ))}
      </div>
      {ctaButton && (
        <div className="mt-8">
          <Button nativeButton={false} className="bg-primary text-on-primary px-8 py-4 font-label uppercase text-sm hover:opacity-80 transition-opacity rounded-[4px]" render={<a href={ctaButton.href} />}>
            {ctaButton.text}
          </Button>
        </div>
      )}

...
export const About3 = ({
  ...
  const hasContentSections = Boolean(contentSections && contentSections.length > 0);
      ...
      <div className="theme-section-wrapper">
          ...
          <div className="flex flex-col gap-7 md:flex-row lg:flex-col">
            <div className="flex flex-col justify-between gap-6 rounded-xl bg-muted p-7 md:w-1/2 lg:w-auto">
              {breakout.src ? (
                <img
                  src={breakout.src}
                  alt={breakout.alt ?? "logo"}
                  className="mr-auto h-12 dark:invert"
                />
              ) : null}

              <div>
                <p className="mb-2 text-lg">{breakout.title}</p>
        ...
        {hasCompanies ? (
          <div className="relative py-32">
            <Marquee className="p-0 [--duration:40s] [--gap:2rem] md:[--gap:4rem]" repeat={2}>
          ...
          <div className="mx-auto grid max-w-5xl gap-16 py-28 md:grid-cols-2 md:gap-28">
            {contentSections!.map((section, idx) => (
              <div key={`${section.title}-${idx}`}>
                <h2 className="mb-5 text-4xl">{section.title}</h2>
...
function MarqueeFade({ side }: { side: "left" | "right" }) {
  return (
    <div
      aria-hidden="true"
      className={cn(
        "pointer-events-none absolute inset-y-0 w-16 md:w-24",
        side === "left"

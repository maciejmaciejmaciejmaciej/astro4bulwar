...
interface NavbarBigSpaceBetweenElementsProps {
  className?: string;
  navbar: GlobalLayoutNavbar;
}

export function NavbarBigSpaceBetweenElements({
  className,
  navbar,
}: NavbarBigSpaceBetweenElementsProps) {
  const [isOpen, setIsOpen] = useState(false);
  const brandLogoAlt = navbar.brand.logoAlt ?? navbar.brand.name;
  const mobileNavItems = navbar.primaryItems;
    ...
    <section className={clsx("bg-white text-on-surface", className)}>
      <div className="page-margin">
        <nav className={clsx("relative transition-colors duration-200", isOpen ? "bg-surface-container-low" : "bg-white")}>
          <div
            className={clsx(
              "relative z-10 px-6 py-6 md:px-10 xl:py-7 transition-colors duration-200 lg:px-0",
              isOpen ? "bg-surface-container-low" : "bg-white",
            )}
          >
            <div className="flex items-center justify-between gap-4 lg:hidden">
              <div className="min-w-[132px]">
                <a href={navbar.brand.href} className="inline-flex items-center transition-opacity hover:opacity-70">
                  {navbar.brand.logoSrc ? (
                    ...
                    <span className="font-headline text-2xl uppercase md:text-3xl">
                      {navbar.brand.name}
                    </span>
                  )}
                </a>
          ...
          <AnimatePresence initial={false} mode="wait">
            {isOpen ? (
              <motion.div
                className="relative z-20 bg-surface-container-low px-6 pb-8 pt-8 shadow-[0_18px_40px_rgba(15,23,42,0.10)] md:px-10 lg:px-0 xl:pb-10"
              ...
              >
                        ...
                        <ul className="mt-3 space-y-2 border-l border-zinc-200 pl-4">
                          {item.children.map((childItem) => (
                            <li key={childItem.label}>
                              <a href={childItem.href} className="font-body text-sm text-secondary transition-colors hover:text-on-surface">
                                {childItem.label}
                              </a>
                            </li>
                          ))}
                ...
                <div className="hidden lg:grid lg:grid-cols-[180px_minmax(0,1fr)_80px] lg:items-start lg:gap-10">
                  ...
                  <div className="grid grid-cols-5 gap-10 xl:gap-14">
                    {navbar.primaryItems.map((section) => (
...
export { NavbarBigSpaceBetweenElements as Navbar2 };

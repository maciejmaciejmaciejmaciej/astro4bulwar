...
- Post-order Make notification: after a Woo order is saved successfully, the bridge sends a best-effort backend-only webhook to Make with the exact payload shape `{ order_id, text_long_string }`, built only from saved Woo order lines, totals, fulfillment meta, and slot timing. Webhook failures are non-blocking and tracked in order meta.

## Health

### `GET /wp-json/bulwar/v1/health`
...
- `remaining_capacity` is returned and filtered by the active engine, but persisted Woo order occupancy is not wired in this step yet. The endpoint exposes `meta.occupancy_source` and `meta.occupancy_limitations` so the client contract stays stable until stricter occupancy validation lands.

...
import {
  type OfertaPostsSectionData,
  type OfertaPostsSectionItem,
  type OfertaPostsSectionSource,
  pageBuilderSchema,
...
const fetchWordPressPostsByIds = async (
  ...
  const response = await fetch(`${getWordPressBaseUrl()}/wp-json/wp/v2/posts?${params.toString()}`, {
    method: 'GET',
    headers: {
  ...
  if (!response.ok) {
    throw new Error(`WordPress posts request failed: ${response.status} ${response.statusText}`);
  }
...
export const resolveOfertaPostsSectionSource = async (
  ...
  const posts = await fetchWordPressPostsByIds(source.sourceValue);

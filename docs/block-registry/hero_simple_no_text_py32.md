...
- block version: 1
- current renderer mapping: `HeroSimpleNoTextPy32`
- current renderer path: `zip/src/components/sections/hero_simple_no_text_py32.tsx`
- implementation status: runtime schema, AI schema descriptor, and TemplatePage preview are implemented

## Purpose

This block renders the existing py32 hero variant as a direct-edit page-builder block.

It keeps:

- one full-width hero image inside the current rounded shell
...
- `max-w-screen-2xl` image wrapper
- rounded image frame
